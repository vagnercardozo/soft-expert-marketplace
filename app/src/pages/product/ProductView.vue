...
import { columnsProduct, Product } from 'src/models/product';
...
const _load = async () => {
  loading.value = true;
  try {
    rows.value = await api.get({ endpoint: 'product/list' });
  } finally {
    loading.value = false;
  }
};
  ...
  await alert('Atenção', 'Deseja mesmo excluir este item?', 'warning').then(
    async (result) => {
      if (result.isConfirmed) {
        await api.remove({ endpoint: `product/delete/?id=${id}` });
        await _load();
      }
    }
  );
...
<template>
  <q-page>
    <div class="title flex justify-center">
      <p class="text-h3 q-ma-xl">Produtos</p>
    </div>
    <div>
      <MTable
    ...
    >
      <FormProduct @close="__closeDialog" :id="editId" />
    </q-dialog>
  </q-page>
</template>
